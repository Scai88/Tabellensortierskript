...
def main():
    ...
    try:
        ...
        for _, row in rechnung_df.iterrows():
            ...
            spendenbetrag = quantize_money(original_brutto_gesamt - berechnetes_brutto)

            brutto_ok = abs(original_brutto_gesamt - berechnetes_brutto - spendenbetrag) <= TOLERANZ
      ...
      # =======================
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            mengen = [
                safe_int(x, default=1)
                for x in str(mengen_text).split("|")
                if str(x).strip()
            ]
            ...
            trinkgeld_row = safe_decimal(get(row, "Trinkgeld €"))
            if trinkgeld_row != Decimal("0.00"):
                auswertung["Spende"][zahlung]["Netto"] += trinkgeld_row
                auswertung["Spende"][zahlung]["Steuer"] += Decimal("0.00")
                auswertung["Spende"][zahlung]["Brutto"] += trinkgeld_row
        ...
        for row in ws.iter_rows(min_row=2):
            sheet_netto += safe_decimal(get(row, "Netto €"))
            sheet_steuer += safe_decimal(get(row, "Steuer €"))
            sheet_brutto += safe_decimal(get(row, "Brutto €"))
            sheet_trinkgeld = safe_decimal(get(row, "Trinkgeld €"))
            sheet_netto += sheet_trinkgeld
            sheet_brutto += sheet_trinkgeld
